...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--help") == 0 && argc == 2) {
            puts("cbm-security-facts --path relative/File.ext [--offset N --limit 1..200]\n"
                 "  [--expect-analysis SHA256] [--fact-id SHA256] < File.ext\n"
                 "--capabilities lists supported languages and bounded framework models.\n"
                 "Reads at most 1 MiB of UTF-8 source from stdin. No files are opened.\n"
                 "Use --expect-analysis for continuation and fact lookup. Facts are not vulnerability verdicts."); return 0;
        }
